...
import { TextParser } from '@/services/parsers/text';
import { JSONParser } from '@/services/parsers/json';
...
export class PlanService {
  ...
  public calculateActuals(node: any) {
    ...
    const properties: Array<keyof typeof NodeProp> = ['ACTUAL_ROWS', 'PLAN_ROWS', 'ROWS_REMOVED_BY_FILTER', 'ROWS_REMOVED_BY_JOIN_FILTER'];
  ...
  public fromJson(source: string) {
    return this.jsonParser.fromJson(source);
  }

  // Just for backward compatiblity.
  public parseJson(source: string) {
    return this.jsonParser.parseJson(source);
  }
